...
function insights(){
  ...
  for(const p of REPS){
    ...
    if((f.closed.length+f.closeLost.length)>=MG && cr < tClose-10){ const tr=topReason(f.closeLost); const xtra=Math.max(1,Math.round((tClose-cr)/100*(f.closed.length+f.closeLost.length)));
      items.push({cat:"rep",imp:f.closeLost.length*10, eur:xtra*ESALE, t:`${p.n} verliest relatief veel dossiers na de show`, p:`Van de afgeronde dossiers na show die ${p.n} bezit is ${fpct(f.closed.length,f.closed.length+f.closeLost.length)} ingeschreven (${f.closeLost.length} verloren${tr?`, vooral "${tr[0]}"`:""}) tegenover ${r1(tClose)}% gemiddeld.`, d:`Kijk naar de verliesredenen na show en het opvolgritme na de intake.`}); }
  }
  const openGS=tot.geenShow.filter(l=>l.open);
  if(openGS.length>=3) items.push({cat:"funnel",imp:openGS.length*4, eur:openGS.length*ERE, t:`${openGS.length} no-shows staan nog open`, p:`Van de ${tot.geenShow.length} geen-shows in deze periode zijn er ${openGS.length} nog niet verloren gezet — die zijn mogelijk nog te herplannen.`, d:`Nabellen/appen en direct een nieuwe intake plannen (klik show-blok → rechterlijst).`});
  if(ts.late.length>=3) items.push({cat:"funnel",imp:ts.late.length*4, eur:ts.late.length*ERE, t:`${ts.late.length} late cancels (op de dag zelf)`, p:`${ts.late.length} van de ${ts.all.length} intakes zijn op de dag zelf geannuleerd — verloren tijd voor de intaker, maar deze mensen zijn makkelijker terug te halen dan een no-show.`, d:`Aparte reactivatie-flow voor late cancels; check of de bevestigingsreminder op tijd gaat.`});
  const wtAll=tot.agenda.filter(l=>l.pd>=0&&l.id_>=0&&(l.is_show||l.is_noshow));
  const wtB=[["zelfde dag",l=>l.id_-l.pd<=0],["1 dag",l=>l.id_-l.pd===1],["2-3 dagen",l=>l.id_-l.pd>=2&&l.id_-l.pd<=3],["4-7 dagen",l=>l.id_-l.pd>=4&&l.id_-l.pd<=7],["8+ dagen",l=>l.id_-l.pd>=8]].map(([n,f])=>{ const xs=wtAll.filter(f); return {n,xs,show:xs.filter(l=>l.is_show).length,sign:xs.filter(l=>l.is_signed).length}; });
  const wtFast=wtAll.filter(l=>l.id_-l.pd<=1), wtSlow=wtAll.filter(l=>l.id_-l.pd>=4);
  if(wtFast.length>=10&&wtSlow.length>=10){ const fs=pct(wtFast.filter(l=>l.is_show).length,wtFast.length), ss=pct(wtSlow.filter(l=>l.is_show).length,wtSlow.length);
    const doel=wtB[2].xs.length>=10?pct(wtB[2].show,wtB[2].xs.length):fs; const extra=Math.max(0,Math.round((doel-ss)/100*wtSlow.length)); const aandeel=pct(wtFast.length,wtAll.length);
    if(fs>ss+8) items.push({cat:"funnel",imp:extra*8, eur:extra*ESHOW, t:`Wachttijd na inplannen kost shows: ${r1(fs)}% vs ${r1(ss)}% show`, p:`Intake binnen 1 dag na inplannen: ${r1(fs)}% show (${wtFast.length}). Intake 4 dagen of later na inplannen: ${r1(ss)}% (${wtSlow.length}). Per wachttijd: `+wtB.map(x=>`${x.n} ${x.xs.length?fpct(x.show,x.xs.length):"—"} show, ${x.sign} getekend (${x.xs.length})`).join(" · ")+`. Nu staat ${r1(aandeel)}% van de intakes binnen 1 dag; ± ${extra} extra shows als de late intakes op het niveau van 2-3 dagen zaten.`, d:`Regel: intake binnen 48 uur na inplannen, streven zelfde dag of morgen (Zoom mag). Doel: 50%+ van de intakes binnen 1 dag. Late slots: dag ervoor bellen om te bevestigen.`}); }
  ...
  for(const r of camp.sort((a,b)=>a.l2i-b.l2i).slice(0,2)) if(r.l2i<teamL2i-8) items.push({cat:"bron",imp:Math.round((teamL2i-r.l2i)/100*r.n)*3, eur:Math.round((teamL2i-r.l2i)/100*r.n)*EPLAN, t:`Campagne "${r.k}" levert weinig intakes op`, p:`${r.gep} van ${r.n} leads kreeg een intake gepland (${r1(r.l2i)}%) tegenover ${r1(teamL2i)}% gemiddeld; ${r1(r.lostShare)}% al verloren, meest "${r.top}".`, d:`Check targeting/creatie van deze campagne of pauzeer hem; vergelijk in tab Bronnen & Ads per advertentie.`});
  for(const r of camp.sort((a,b)=>b.l2i-a.l2i).slice(0,1)) if(r.l2i>teamL2i+8) items.push({cat:"bron",imp:Math.round((r.l2i-teamL2i)/100*r.n)*2, eur:0, good:true, t:`Beste campagne: "${r.k}"`, p:`${r1(r.l2i)}% van de ${r.n} leads kreeg een intake gepland (gemiddeld ${r1(teamL2i)}%); ${r.sign} ingeschreven.`, d:`Schaal budget hierheen als de kosten per lead vergelijkbaar zijn.`});
  for(const r of kan) if(r.n>=30&&r.l2i<teamL2i-10) items.push({cat:"bron",imp:Math.round((teamL2i-r.l2i)/100*r.n)*2, eur:Math.round((teamL2i-r.l2i)/100*r.n)*EPLAN, t:`Kanaal ${r.k}: lage lead-kwaliteit`, p:`${r1(r.l2i)}% van ${r.n} leads kreeg een intake (gemiddeld ${r1(teamL2i)}%); top verliesreden "${r.top}".`, d:`Kwalificatievragen aanscherpen of ander publiek testen op dit kanaal.`});
  ...
  if(wk.length>=5){ const last=wk[wk.length-1], prev=wk.slice(-5,-1); const avg=k=>prev.reduce((s,r)=>s+r[k],0)/prev.length;
    for(const [k,lab] of [["pr","plan rate"],["sr","show rate"],["gs","sign rate"],["cr","close rate"],["l2s","lead → sale"]]){ const a=avg(k), v=last[k]; if(a>0&&Math.abs(v-a)>=8){ const TB={pr:[r=>r.beh,EPLAN],sr:[r=>r.agenda,ESHOW],gs:[r=>r.show,ESALE],cr:[r=>r.closed+r.closeLost,ESALE],l2s:[r=>r.beh,SALE]}; const den=TB[k][0](last)||0, mis=v<a?(a-v)/100*den*TB[k][1]:0; items.push({cat:"trend",imp:Math.abs(v-a)*3, eur:mis, good:v>a, t:`${lab.charAt(0).toUpperCase()+lab.slice(1)} vorige week ${v<a?"gezakt":"gestegen"} naar ${r1(v)}%`, p:`Gemiddelde van de 4 weken ervoor: ${r1(Math.round(a*10)/10)}%${v<a?`. Op ${den} in die week is dat ± ${eur(mis)} minder omzet`:""}.`, d:`Zie tab Trend voor het verloop per week en per persoon.`}); } } }
...
function drawAsk(){
  ...
  h+=`<div class="askhead"><b>⚡ Adviezen</b><span>${fmtY(A)} t/m ${fmtY(B)}${askFilter!=="all"?" · focus: "+catLab[askFilter]:""} · ${list.length} van ${I.items.length} signalen · gesorteerd op geschatte gemiste omzet · klik een rij voor uitleg en actie</span><span style="flex:1"></span><button id="pakbtn" class="chip" onclick="askPak()">📋 Kopieer datapakket voor Claude</button></div>`;
  ...
  <p class="note"><b>Hoe de volgorde bepaald wordt.</b> Elk signaal krijgt een bedrag: het aantal stappen dat je tegenover het teamgemiddelde bent misgelopen, maal de kans dat zo'n stap tot een handtekening leidt, maal € ${(+(DEFS.omzet_per_student||6800)).toLocaleString("nl-NL")} per traject. Die kansen komen uit deze periode zelf: een gemiste geplande intake telt voor ${r1(Math.round(I.tShow*I.tSign)/100)}% van een sale, een gemiste show voor ${r1(I.tSign)}%, een gemiste inschrijving voor 100%. Bij al verloren leads rekenen we met ${Math.round(100*(+(DEFS.terughaal_aandeel_verloren||0.2)))}% terughaalkans, bij open no-shows met de gemeten show rate bij herplanning. Bovenaan staat dus wat je deze periode het meeste geld kost. Trendrijen gaan over één week (de laatste afgeronde), de rest over de gekozen periode. Groen = goed nieuws, die krijgen geen bedrag.</p>`;
  aw.innerHTML=h;
}
