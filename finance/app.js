  ...
  return `<div class="wlrow ${cls}" onclick="wlTog(${JSON.stringify(String(k)).replace(/"/g,"&quot;")})">
    <div class="wlhead">${rank?`<span class="rank">${rank}</span>`:`<span class="rank">·</span>`}
      ...
      <span class="wlmeta"><span>${d.late>0?`<b style="color:var(--red-tx)">${d.late} dgn te laat</b>`:"nog niet vervallen"}</span>
        <span>${d.nOpen} open factuur${d.nOpen===1?"":"en"}</span>
        ${cand.length?`<span><span class="stg warn">⚠︎ ${cand.length} betaling${cand.length===1?"":"en"} nog niet afgeletterd</span></span>`:""}</span>
    ...
    <div class="why">${esc(d.actT)} — ${esc(d.why)}${cand.length?" · eerst afletteren checken!":""}</div>
    ...
    <div><h4>Betalingen nog niet afgeletterd</h4>${cand.length?cand.map(c=>`<div class="lr"><span title="${esc(c.t.ref||"")}">${eur0(c.t.amount)} · ${fmt(c.t.date)} · ${esc(c.why[0]||"")}</span></div>`).join("")+`<div style="margin-top:8px"><span class="okbtn" onclick="event.stopPropagation();reconGo(${JSON.stringify(achternaam(d.nm)).replace(/"/g,"&quot;")})">🔗 Open bankaflettering (zoekterm gekopieerd)</span></div>`:`<div class="lr"><span>Geen onafgeletterde betalingen gevonden voor deze leerling.</span></div>`}<div class="note" style="margin-top:8px">Betaalafspraken uit e-mail volgen in fase 2.</div></div></div>`:""}
  </div>`;
}
...
function afletHtml(){
  ...
  h+=`<div class="wl">`+withPay.map(c=>{
    ...
    return `<div class="wlrow ${d.open>0?"mid":""}" onclick="afTog(${JSON.stringify(k).replace(/"/g,"&quot;")})">
      ...
      ${opn?`<div class="wlx"><div><h4>Gevonden betalingen</h4>${c.cand.map(x=>`<div class="mtch"><span class="conf ${x.sc>=70?"hi":x.sc>=45?"mid":"lo"}">${x.sc>=70?"zeker":x.sc>=45?"waarschijnlijk":"onzeker"}</span><span><b>${eur0(x.t.amount)}</b> · ${fmt(x.t.date)}${x.t.pid?' · <span class="stg win">partner staat erop</span>':""}<br><span class="chsub" title="${esc(x.t.ref||"")}" style="display:inline-block;max-width:480px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:middle">"${esc(x.t.ref||"—")}"</span><br><span class="chsub">${x.why.join(" · ")}</span>${(h=>h.length?`<br><span class="chsub">🔎 controle — dit rekeningnummer betaalde eerder: ${h.map(v=>eur0(v.amount)+" op "+fmt(v.date)+(v.rec?" (afgeletterd"+(v.pname?" op "+esc(v.pname):"")+")":" (nog open)")).join(" · ")}</span>`:"")(ibanHist(x.t))}</span>${!x.t.pid&&d.pid?`<span class="act"><button class="okbtn" onclick="setPartner(${x.t.id},${d.pid},event)" ${busySet.has(x.t.id)?"disabled":""}>${busySet.has(x.t.id)?"⏳ bezig…":"👤 Zet "+esc(achternaam(d.nm))+" als partner in Odoo"}</button></span>`:""}</div>`).join("")}</div>
      <div><h4>Facturen van ${esc(d.nm)}</h4><table><tr><th>Nr</th><th>Bedrag</th><th>Open</th><th>Status</th></tr>${d.inv.map(i=>`<tr><td onclick="event.stopPropagation()">${olink("account.move",i.id,esc(i.name||"—"))}</td><td>${eur0(i.total)}</td><td><b>${i.open>0?eur0(i.open):"✓"}</b></td><td>${psPill(i)}</td></tr>`).join("")}</table><div class="note" style="margin-top:8px">Werkwijze: 1) check de betaling hier — bedrag, omschrijving, de redenen en de 🔎 controle-regel (wat dit rekeningnummer eerder betaalde). Twijfel je, klik dan eerst de naam of factuur open in Odoo. 2) klopt het → "Zet … als partner in Odoo". Dit zet alléén de klantnaam op de betaling (geen boeking; in Odoo zo weer te wijzigen). 3) open de Bankaflettering: Odoo toont nu zelf de Afletteren-knop met de juiste factuur — dáár gebeurt pas het echte boeken, door jou.</div></div></div>`:""}
    </div>`;
  }).join("")+`</div></div>`;
